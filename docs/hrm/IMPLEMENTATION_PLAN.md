...
- [x] `make lint test` passes (no compile errors, no test failures) — **DONE 2026-04-22** (`pnpm tsc --noEmit` 0 errors in HRM files; `pnpm lint` ✔ no warnings or errors; Go build clean)
- [x] E2E browser verification — **DONE 2026-04-22** (see §E2E Results below)

### E2E Results (2026-04-22)

Tested as `admin@mdh.vn` (SUPER_ADMIN) via Playwright MCP Docker browser against `host.docker.internal:3000`.

| Page | Result | Notes |
|------|--------|-------|
| `/admin/hrm/employees` | ✅ PASS | List renders seed employee NV26-0001; search input present; pagination present |
| `/admin/hrm/employees/new` | ✅ PASS | Form renders all fields; grade/status/employment_type/work_location/gender/education dropdowns work |
| Create employee (submit) | ✅ PASS | Created "Tran Thi B" (NV26-0002); redirected to detail page `/admin/hrm/employees/:id` |
| Detail — Info tab | ✅ PASS | All fields displayed; employee_code auto-generated by trigger |
| Detail — Dependents tab | ✅ PASS | Empty state "Chưa có người phụ thuộc" with Thêm button |
| Detail — Hợp đồng tab | ✅ PASS | Empty state "Chưa có hợp đồng" with Thêm hợp đồng button |
| Detail — Lịch sử lương tab | ✅ PASS | Empty → added 12,000,000 VNĐ INITIAL entry → table row renders with badge |
| PII button — step 1 (confirm dialog) | ✅ PASS | "Truy cập dữ liệu nhạy cảm" dialog with audit warning rendered |
| PII button — step 2 (decrypt view) | ✅ PASS | Decrypted modal shows all PII fields (empty for new employee); access timestamp shown |
| Audit log written on PII access | ✅ PASS | `EMPLOYEE_PII_ACCESSED` in `audit_logs` table confirmed via DB query |
| `/my-profile` (admin has no employee record) | ✅ PASS | Graceful empty state: "Chưa có hồ sơ nhân viên. Liên hệ HR để tạo hồ sơ." |
| Console errors | ⚠️ WARN | Only Radix UI `aria-describedby` warnings on dialogs (cosmetic, not functional) |
...
**TypeScript check:** 0 errors in HRM files (`pnpm tsc --noEmit` — pre-existing `commissions/my/page.tsx` error fixed as part of Sprint 1 Day 2 Phase 4)
**ESLint:** `pnpm lint` → ✔ No ESLint warnings or errors (pre-existing `@typescript-eslint` misconfiguration in `.eslintrc.json` fixed)

### Deviations from SPEC (Sprint 1)

| # | SPEC Reference | Deviation | Impact | Resolution |
|---|---|---|---|---|
| D1 | §11.9 `salary_at_signing` | SPEC shows TEXT (for encryption); migration used NUMERIC (plaintext) | Low — no PII in salary amount | Deferred: future migration to encrypt if required |
| D2 | §14.1 Employee detail tabs | SPEC describes 5 tabs (Info, Personal/PII, Salary, Contracts, Insurance); implemented 4 tabs — PII access via dedicated PII button instead of dedicated tab | Low — PII accessible, just via button instead of tab; cleaner UX | Accepted as intentional UX deviation |
| D3 | §13.11 contracts | SPEC includes `POST /hrm/employees/:id/contracts/:cid/set-current` and `GET /hrm/contracts/expiring?days=30`; not yet implemented | Medium — cannot set current contract or view expiring | Deferred to Sprint 1 Day 7 task (task 7.3) |
| D4 | §13.16 insurance-config | `GET/POST /hrm/insurance-config` endpoints not yet implemented | Low — config seeded via migration | Deferred to Sprint 1 Day 7 task (task 7.4) |
| D5 | §18.1 Encryption key | SPEC specifies single `HRM_ENCRYPTION_KEY`; implementation uses same key for all PII fields via `pkg/crypto/aesgcm.go` | Low — key material equivalent | Accepted; no separate key-per-field required |

### Tech Debt (Sprint 1)

| # | Item | Severity | Tracked |
|---|---|---|---|
| TD1 | `dependent-section.tsx` — dependent PII fields (cccd_or_birth_cert) are plaintext text inputs, not encrypted | Low | Sprint 2 |
| TD2 | `contract-section.tsx` — `salary_at_signing` is NUMERIC, no encryption | Low | Sprint 2 (pending D1 resolution) |
| TD3 | Radix UI `aria-describedby` missing on dialogs — console warnings in browser | Cosmetic | Sprint 2 cleanup |
| TD4 | `apps/web/.eslintrc.json` — `@typescript-eslint` rules were missing the plugin; fixed by removing rules | Fixed 2026-04-22 | — |
| TD5 | `commissions/my/page.tsx` — `LucideIcon` type incompatibility with `@types/react@18.3.28` bigint; fixed via import change | Fixed 2026-04-22 | — |
| TD6 | `apps/web/.env.local` — `NEXT_PUBLIC_API_URL=http://host.docker.internal:8080` added for E2E browser testing; must be removed or overridden for production builds | Dev-only | Remove before staging deploy |
